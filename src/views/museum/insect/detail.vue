    ...
    <template v-slot:default="{ detailInfo }">
      <v-list dense class="pt-8 pb-16">
        <v-row>
          ...
          <v-col v-if="detailInfo.weatherCondition" cols="3" class="py-2">
            <v-list-item>
              <v-list-item-content class="label"
                >天气条件：</v-list-item-content
              >
              <v-list-item-content class="align-end">{{
...
export default {
  ...
  methods: {
    dataProcess(data) {
      ...
      const elseCondition = data.elseCondition
      elseCondition.forEach((e, i) => {
        elseData += '(' + e + ')'
      })
      data.locale = localeData.join('、') + elseData
    },
  },
}
</script>
